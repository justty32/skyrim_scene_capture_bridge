...
- **狀態**：✅ **P1–P6 核心功能已存在**；部分 post-P6／ghost 回歸仍 open，不能視為全部實機驗收完畢。面板：F1 → `Scene Capture Bridge`。驗收明細見 [landed/world.md](../ModForge/workflows/feature-dev/landed/world.md)；未驗項見 [wait_todo/ingame-tests.md](../ModForge/wait_todo/ingame-tests.md)。
...
> **尚無實機驗收記錄（2026-08-26）**：`75308c9` 讓 `sc pl py0` 的 frozen palette placement 經 editor commit／cancel 後仍保持 frozen；`a17e460` 讓 ghost 與 gh0 direct placement 改走最終 rendered `NiCamera` ray，對齊第一／第三人稱與 SmoothCam，並略過玩家；`5273576` 讓 movable preview ghost 的 deferred freeze 重試到 motion change 真正成功，且 placement ray 略過 live／sentinel ghost，避免 follow self-hit。這三項都已提交，但都**尚未有 Skyrim 實機 PASS 記錄**。
