...
- **離線 catalog consumer**：把 ModForge `scene-catalog.json` v1 放到 SKSE 資料目錄（與 `scene-capture-palette.json`、匯出檔同層），下次啟動時 Browser 會按 durable FormKey 補 EditorID 與 runtime 缺少的名稱，納入搜尋/顯示；缺檔或壞檔安全退回 runtime-only。離線資料只是 metadata，不會增加遊戲未載入的 form，也不取代 runtime model/name；目前尚未核對 catalog source SHA-256 是否等於實際 load order 檔案。
...
| `CatalogFile.{h,cpp}` | **ModForge `scene-catalog.json` v1 的 portable consumer core**：驗 schema version、完整 source/record shape、SHA-256/FormKey 格式與未知欄位；以大小寫不敏感的 durable FormKey 建唯一索引，重複 key 明確拒絕；`Enrich` 保證 runtime 非空 name/model 優先。無 RE/SKSE 相依，`tests/CatalogFileTests.cpp` 可用 MinGW + CTest 驗證。`Catalog.cpp` 在 kDataLoaded 從 SKSE 資料目錄載入，Browser 首開掃 runtime form 時 merge；缺檔/壞檔 fail-soft。 |
