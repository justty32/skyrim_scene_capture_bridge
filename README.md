...
- **離線 catalog consumer（已完成；portable suite 2/2 PASS）**：把 ModForge `scene-catalog.json` v1 放到 SKSE 資料目錄（與 `scene-capture-palette.json`、匯出檔同層），下次啟動時 Browser 會按 durable FormKey 補 EditorID 與 runtime 缺少的名稱，納入搜尋/顯示。`Catalog.cpp` 已接上載入、compatibility gate 與 metadata merge，`CatalogFile.cpp` 是不依賴 SKSE 的 portable parser；`catalog_file_tests` 與 `modforge_catalog_contract` 均通過。catalog 必須由**完整 resolved load order** 建立：來源集合與 engine `TESDataHandler::files` 過濾出的 loaded global sequence（full + light）需完全一致，且 provenance/record cross-reference 合法；否則整份忽略並安全退回 runtime-only。離線資料只是 metadata，不會增加遊戲未載入的 form，也不取代 runtime name/model。SHA-256 仍未 runtime 核對，Browser 會明示。
- **狀態**：✅ **P1–P6 核心功能已存在**；部分 post-P6／ghost 回歸仍 open，不能視為全部實機驗收完畢。面板：F1 → `Scene Capture Bridge`。驗收明細見 [landed/world.md](../ModForge/workflows/feature-dev/landed/world.md)；未驗項以本 repo 的 [ghost spin 調查](GHOST_SPIN_INVESTIGATION_2026-08-25.md)、[ghost cell clear 調查](GHOST_CELL_CLEAR_INVESTIGATION_2026-08-26.md)與[下方待實機清單](#尚無實機驗收記錄2026-09-05)為準。ModForge 側的 `wait_todo` 尚未同步，待該 repo 的線補。
...
| `SceneExporter.{h,cpp}` | **核心**：`ExportCell` 走訪 cell → **vanilla diff ＋ ownership gate**（ref 解得出耐久 id ⇒ 既有，依 registry 分流到 `removals[]`／`overrides[]` 或跳過；解不出耐久 id 的 dynamic ref 也只有 `Palette::PlacedInfoFor` 命中才 emit，engine 自己 `PlaceAtMe` 的魚／critter 等一律拒絕）→ `placements[]`（**只有物件**：actor 掃描時排除，2026-07-12 拍板）；`ExportCaptures` 另出 captures 檔；`ResolveDurableId` FormID→`<plugin>:0xLOCALID`；`WriteSceneFile` 吐 json（檔名帶場景＋時間戳） |
| `CatalogFile.{h,cpp}` | **ModForge `scene-catalog.json` v1 的 portable consumer core**：驗 schema version、完整 source/record shape、SHA-256/FormKey 格式與未知欄位；以大小寫不敏感的 durable FormKey 建唯一索引，重複 key 明確拒絕；`AssessCompatibility` 擋 source 重複/斷序、FormKey/plugin 或 record/source cross-reference 錯誤、catalog/runtime plugin 集合或 global order 不一致；`Enrich` 保證 runtime 非空 name/model 優先。無 RE/SKSE 相依，`tests/CatalogFileTests.cpp` 可用 MinGW + CTest 驗證；`tests/RunModForgeCatalogContract.cmake` 另會呼叫 sibling ModForge 真實 CLI，以 synthetic full+light plugin 的 exporter bytes 串進 consumer，防跨 repo 契約漂移；`catalog_compatibility_probe` 可把真實 catalog 與 resolved load-order paths 餵進同一 consumer gate。`Catalog.cpp` 在 kDataLoaded 從 SKSE 資料目錄載入，以 `TESDataHandler::files` 保留 full/light 全域順序，Browser 首開掃 runtime form 時 merge；一般 base 讀 `TESModel`，ARMO 改讀 `TESBipedModelForm::worldModels`（含 template fallback），不再被 generic model gate 全數剔除；缺檔/壞檔/相容性不合 fail-soft。 |
| `UI.{h,cpp}` | 遊戲內面板（[SKSE Menu Framework 3](../../analysis/mod-survey/findings/skse-menu-framework-3.md) / Dear ImGui）：顯示所在 cell、Export 按鈕、上次匯出統計；Eraser/Palette/Editor/Markers 各頁帶 **this cell only 過濾**與逐列 undo/revert/del。**2026-07-12 清掉「按一下就執行世界動作」的按鈕**（place marker here／erase by ray／pick by ray／capture crosshair・by ray／select by ray／cancel (restore)）——這批動作全走 `sc` console 指令＋每模式動作鍵（P5 之後 UI 觸發是多餘的），面板只留設定/檢視/清單類。**軟相依**——`IsInstalled()` 是 `GetModuleHandleW` 探測，沒裝框架就只有 hotkey。Palette 頁的插槽改名欄改用 `UI.Fields` 的 bound field（Enter／點開別處都會 commit）。**Eraser／Captures／Editor-overrides／Palette 四頁現在每列都有 label＋note 的 bound field＋apply 鈕**（同樣走 `UI.Fields`）；Eraser／Overrides 兩頁的列改用**耐久 id 的 hash** 定位（不用 list 位置），上面一列 undo/revert 不會把下面列的欄位錯位到別筆 entry |
...
| `Aim.{h,cpp}` | 共用視角射線：`LookHit()`／`RayRef()` 保留 player-facing 老手感；placement-only `RenderedCameraHit()` 從最終 NiCamera 射出，讓 ghost／gh0 direct placement 對齊第一／第三人稱與 SmoothCam。它的 broad collector 明確略過 player 與 live/sentinel ghost，避免每幀 follow 自我命中；已提交 real ref 仍是普通可瞄 world surface。**射線絕不做自動 fallback**（使用者拍板 2026-07-11）——牆/地板都是 ref，自動 fallback 會把「按空」變誤抓；射線只走明示按鈕/專用鍵 |
...
| `Physics.{h,cpp}` | havok 凍結/放開的共用原語（`HavokMovable` 判定 ＋ `FreezeDeferred`：3D root 出現後仍持續重試，直到 child rigid body 接受 `SetMotionType(kKeyframed)`）。Markers/Editor/Palette/Preview 共用（原本各抄一份）。**⚠️ 這一層是 runtime、進不了 esp**——真正 ship 的是 `noHavokSettle` 記錄旗標 |
...
> **尚無實機驗收記錄（2026-09-05）**：`75308c9` 讓 `sc pl py0` 的 frozen palette placement 經 editor commit／cancel 後仍保持 frozen；`a17e460` 讓 ghost 與 gh0 direct placement 改走最終 rendered `NiCamera` ray，對齊第一／第三人稱與 SmoothCam，並略過玩家；`5273576` 讓 movable preview ghost 的 deferred freeze 重試到 motion change 真正成功，且 placement ray 略過 live／sentinel ghost，避免 follow self-hit；`21867c1` 在玩家跨 cell 時重建 preview。2026-09-05 的 `7a95a14` 修正 ghost commit 丟失 slot extra／actor 身分、`51e442d` 修正 Browser 借用的 `gh1` 洩漏、`61d969d` 修正 physics release 誤報成功、`bb13695` 修正 palette 刪除後選擇錯位、`a8512f0` 修正 editor cancel 未更新縮放、`f5efc7f` 修正失敗 slot 留著舊 ghost、`e6e927b` 修正失敗 slot 恢復後 ghost 不回來、`0e1d6aa` 與 `4c15f0c` 補齊 Eraser／marker dynamic ref 的刪除標記。以上皆已提交，但都**尚未有 Skyrim 實機 PASS 記錄**。

## 尚未做（依 spec 里程碑）

- **A 線仍有三項工作**：A1 是唯一的**未知技術 spike**（`Inventory3DManager` 面板內真 3D 預覽）；A2「施法即編輯」尚無 `TESSpellCastEvent` 實體；A3 favorite UI（加入最愛語意、星號／置頂）也尚無實體。A1 不是唯一未做工作。
